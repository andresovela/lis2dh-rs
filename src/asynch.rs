    ...
    I2C: embedded_hal_async::i2c::I2c + embedded_hal::i2c::ErrorType<Error = E>,
{
    ...
    pub async fn set_interrupt_reference(&mut self, value: u8) -> Result<(), Error<E>> {
        self.write_register(Register::Reference, value).await
    }
    ...
    async fn enable_click_interrupt(&mut self, config: &ClickIntConfig) -> Result<(), Error<E>> {
        ...
        if config.high_pass_filter {
            self.set_register_bits(Register::CtrlReg2, HPCLICK).await?;
        } else {
